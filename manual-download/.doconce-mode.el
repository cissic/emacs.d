...
;; M-x DocOnce   ; to enable doconce mode!
...
;; (local-set-key (kbd "\C-c\C-p") 'make-pdf-in-shell)
;; )
...
(defun save-and-bash-make ()
  (interactive)
  (save-some-buffers 1)
  (async-shell-command "bash make.sh"))
(global-set-key [f5] 'save-and-bash-make)
(global-set-key (kbd "\C-c\C-b") 'save-and-bash-make)

; add commenting using newcomment.el:
...
(set-face-attribute 'do-comment-face nil :foreground "purple")
